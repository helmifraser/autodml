...
class CarlaGame(object):
    def __init__(self, carla_client, args):
        ...
        self._measurements_file = 0
        self._reset = False
        self._player_start_spots = 0
    ...
    def _save_data(self, measurements, sensor_data):
        for name, measurement in sensor_data.items():
            filename = "{}_{}_{}.png".format(self._frame_counter, name, self._episode_counter)
            measurement.save_to_disk(filename)

        self._measurements_file.write("{} {} {} {} {} \n".format(self._frame_counter,
                ...
                measurements.player_measurements.autopilot_control.reverse))

    def _cleanup_data(self, folder_name):
    ...
    def _frame_limit(self, current_frame):
        if current_frame == FRAMES_PER_EPISODE:
            return True
    ...
    def execute(self):
        ...
        try:
            while self._episode_counter < self._player_start_spots:
    ...
    def _initialize_game(self, init_pos=0):
        self._on_new_episode(init_pos)
    ...
    def _on_new_episode(self, player_start=0):
        scene = self.client.load_settings(self._carla_settings)
        self._player_start_spots = len(scene.player_start_spots)
        self._map_name = scene.map_name
        if self._display_map:
            self._city_name = scene.map_name

        if player_start > self._player_start_spots:
            print("Error: invalid start position! Start position doesn't exist.")
            sys.exit()

        if player_start == EPISODE_LIMIT:
            print("\nUser defined episode limit reached")
            sys.exit()

        # number_of_player_starts = len(scene.player_start_spots)
        # player_start = np.random.randint(number_of_player_starts)

        # 0 on town 1 is a straight line, 1 has turns
        # player_start = 1

        print('Starting new episode: {}\n'.format(self._episode_counter))
        self.client.start_episode(player_start)
        self._timer = Timer()
        self._is_on_reverse = False
        self._frame_counter = 0
        self._measurements_file = open("{}_episode.txt".format(self._episode_counter),"wa+")
    ...
    def _on_loop(self):
        ...
        self._save_data(measurements, sensor_data)
        ...
        reset = self._frame_limit(self._timer.step)

        if control is None or reset is True:
            self._cleanup_data(str(self._episode_counter))
            self._episode_counter += 1
            self._on_new_episode(self._episode_counter)
